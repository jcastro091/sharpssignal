...
function publicMlbUnderdogProbation(rows, shadowRows = []) {
  ...
  return {
    ...
    current_best_retail_book: clean(liveLane?.current_best_retail_book) || clean(liveFallback?.current_best_retail_book),
    minimum_bettable_odds: clean(liveLane?.minimum_bettable_odds) || clean(liveFallback?.minimum_bettable_odds),
...
export async function loadPicksPreviewFromSupabase({ timezone = DEFAULT_TZ, debug = false } = {}) {
  ...
  const { data: shadowRows, error: shadowError } = await safeSelect("model_predictions", (signal) =>
    supabase
      ...
      .select("observed_at,sport,away_team,home_team,market,pick_side,official,game_time,prediction_result,clv_pct,closing_odds_american,updated_at,odds_american,raw")
      .eq("official", false)
      .gte("observed_at", todayStart)
      .lt("observed_at", tomorrowStart)
      .order("observed_at", { ascending: false })
