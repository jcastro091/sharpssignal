...
export type HistoricalPick = {
  ...
  pick?: string; // "Boston Celtics ML" / "Milwaukee Bucks -3.5" etc.
...
function resolveDateHint(dateHint: string): { dash: string; ymd: string } {
  ...
  // Try to parse explicit YYYY-MM-DD
...
// Determine if a row is an actual AI pick (A/B/C tier etc.)
function isPickRow(row: CsvRow): boolean {
  ...
  if (tierLabel.includes("tier") && !tierLabel.includes("pass")) return true;
...
// Map a CSV row into our HistoricalPick structure
function rowToHistoricalPick(row: CsvRow): HistoricalPick {
  ...
  for (const col of ["American Odds", "Odds (Am)", "LowVig Home Odds (Am)", "LowVig Away Odds (Am)"]) {
    const v = row[col];
  ...
  // Build a human-ish pick string (H2H only for now)
  ...
  return {
    ...
    ev_percent: edge != null ? edge * 100 : null, // convert 0.25 -> 25%
    kelly_pct: null, // optional: derive from Stake Amount + bankroll if you want
    reason,
    movement,
    sport,
    startTimeISO,
...
// ----- Public API used by /api/assistant -----
...
export async function findHistoricalPick(args: {
  ...
  try {
    ...
    // Latest pick = latest Timestamp among pick rows
    pickRows.sort((a, b) => {
...
// Optional helper if other routes want “today’s pick” directly
export async function getPickForQuery(q: string): Promise<HistoricalPick | null> {
  // Simple heuristic: anything mentioning yesterday goes to yesterday,
  // everything else -> today
  const s = q.toLowerCase();
  const hint = s.includes("yesterday") || s.includes("last night") ? "yesterday" : "today";
